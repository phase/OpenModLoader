...
public class MaterialRecipeShapeless implements IRecipe {

    private final List<Set<ItemStackWrapper>> input;
    private final ItemStack output;

    public MaterialRecipeShapeless(ItemStack output, Object... input) {
    ...
    public boolean matches(InventoryCrafting var1, World var2) {
        List<Set<ItemStackWrapper>> l = Lists.newArrayList(input);
        label1:
        for (int i = 0; i < var1.getSizeInventory(); i++) {
            ...
            for (int i2 = 0; i2 < l.size(); i2++) {
                ...
                else if (l.get(i2).contains(new ItemStackWrapper(stack))) {
                    l.remove(i2);
                    continue label1;
                }
            }
